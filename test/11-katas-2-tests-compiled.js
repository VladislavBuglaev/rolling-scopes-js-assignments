...
describe('11-katas-2-tasks', function () {
    ...
    it.optional('parseBankAccount should return the bank account number from the specified string', () => {
        [{
            ...
            assert.equal(tasks.parseBankAccount(data.text), data.result, `${ data.text } has not parsed correctly:`);
        });
    });
    ...
    it.optional('wrapText should return the sequence of lines from the specified string', () => {
        ...
        [{
            cols: 26,
            expected: ['The String global object', 'is a constructor for', 'strings, or a sequence of', 'characters.']
        }, {
            cols: 12,
            expected: ['The String', 'global', 'object is a', 'constructor', 'for strings,', 'or a', 'sequence of', 'characters.']
        }, {
            ...
            assert.deepEqual(Array.from(tasks.wrapText(text, data.cols)), data.expected, `'${ text }' has not wrapped correctly for ${ data.cols } columns:`);
        });
    });
    ...
    it.optional('getPokerHandRank should return the rank of the specified poker hand', () => {
        ...
        [{
            ...
            assert(actual >= PokerRank.HighCard, 'Invalid return value. The return value should be >= PokerRank.HighCard');
            assert(actual <= PokerRank.StraightFlush, 'Invalid return value. The return value should be <= PokerRank.StraightFlush');
            assert(actual == data.expected, `'${ data.hand }' is ranked as ${ rankNames[data.expected] }, but actually ${ rankNames[actual] } `);
        });
    });
    ...
    it.optional('getFigureRectangles should return the sequence of rectagles parts of the specified figure', () => {
        [{
            ...
            assert.deepEqual(actual, expected, `Figure \n${ data.figure } has the following parts:\n${ expected.join(',\n') } but actually :\n${ actual.join(',\n') }`);
        });
    });
});

//# sourceMappingURL=11-katas-2-tests-compiled.js.map